...
public class Driver {
	public static void main(String[] args) throws Exception {
		...
		// creates the list of items to insert, where the index is the value
		// and the entry is the priority.
		for (int i = 0; i < n; i++) {
			priority = rand.nextInt(universe);
			val = rand.nextInt(universe);
			
			// checks to make sure no duplicates
		...
		// push the randomized list of operations onto each stack
		for (int i = 0; i < m; i++) {
			temp = rand.nextInt(3);
			stk1.push(temp);
			stk2.push(temp);
		...
		for (int i = 0; i < m ; i++) { // loops through doing the different operations
			selection = stk1.pop();
			switch (selection) { // depending on the random operation popped from the stack, do that op
			case 0:
				bhqueue.ExtractMax();
				break;
			case 1:
				...
				break;
			case 2:
				priority = rand.nextInt(bhqueue.getSize()) + universe; // added just to make sure that increase will always increase
				val = rand.nextInt(universe);
	...
	/**
	 * checks to make sure that the list a doesn't contain value x 
